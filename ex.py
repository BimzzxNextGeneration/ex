...
username = "extrash"
password = "extrash"
limitattack = "UNLIMITED"

for i in range(3):
	os.system('cls' if os.name == 'nt' else 'clear')
	print("hayuuu passwort ama usernemnya afah :<\n")
	usr = getpass.getpass(prompt='username@login: ~$ ')
	pwd = getpass.getpass(prompt='password@login: ~$ ')
	j=3
	if pwd == password and usr == username:
		time.sleep(2)
		print("[√] Accept, Your Account Valid.")
		time.sleep(3)
		break
	else:
		time.sleep(2)
		print("\u001b[31m[x] Do You Have Account? | Your Password or Username Invalid !!\033[0m")
		time.sleep(3)
		continue
...
def main():
	...
	while True:
		...
		sys.stdout.write("\x1b]2;[@] ExtrashTools. | User & Password: [root@{}] | Limit Attacks: [{}] | Power [{} %]\x07".format(username, limitattack, x))
		sin = input("root@{}: ~$ ".format(username))
		sinput = sin.split(" ")[0]
		if sinput == "clear":
			os.system('cls' if os.name == 'nt' else 'clear')
			print(banner)
